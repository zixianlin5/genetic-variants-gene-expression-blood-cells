...
# Function to extract the chromosome number from the SNP position
...
for (CHR in keys_list) {
  ...
  missing_snps_not_found <- list()  # List to store missing SNPs not found in genotype_data for each CHR
  ...
  reverse_snps_data <- genotype_data[, c("IID", reverse_snp_list), 
                                     with = FALSE, nomatch = 0]
  ...
  if (length(missing_snp_columns) > 0) {
    ...
    for (snp in missing_snp_columns) {
      ...
      # Get SNP prefix (before the underscore)
      snp_prefix <- strsplit(snp, "_")[[1]][1]
      ...
      if (!is.null(ref_allele)) {
        ...
        if (reverse_snp %in% colnames(reverse_snps_data)) {
          ...
          # Merge into snp_data
          snp_data <- merge(snp_data, temp_data, by = "IID", all.x = TRUE)
          
        } else {
          # Store missing SNP not found in genotype_data
          missing_snps_not_found <- c(missing_snps_not_found, snp)
          cat(paste0("  reverse_snp not found in genotype_data: ", reverse_snp, "\n"))
        }
  ...
  # Print missing SNPs for the current CHR if any
  if (length(missing_snps_not_found) > 0) {
    cat(paste0("[!] The following SNPs were not found in genotype_data for CHR", CHR, ":\n"))
