...
log_suffix <- args[1]
log_name <- sprintf("fit_models_interaction_coeffs_log_%s.txt", log_suffix)
...
process_gene <- function(i, assigned_genes, start_time) {
  ...
  result <- tryCatch({
    withCallingHandlers({
      ...
      coefs <- summary(interaction_model)$coefficients
      
      sig_coefs <- coefs[,"Pr(>|z|)"] < 0.05
      sig_num_sig_coefs <- sum(sig_coefs)
      ...
      p_values_interactions <- coefs[sig_interactions, "Pr(>|z|)"]
      
      model_data <- list(
        gene = this_gene,
        
        fixed_variables = rownames(coefs),
        estimated_coefficients = coefs[, "Estimate"],
        std_errors = coefs[, "Std. Error"],
        z_values = coefs[, "z value"],
        p_values = coefs[, "Pr(>|z|)"],
        ...
        p_values_interactions = p_values_interactions,
...
save_data_to_rds(final_results, "interaction_coeffs")
