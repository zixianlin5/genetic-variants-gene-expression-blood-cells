    ...
    withCallingHandlers({
      ...
      if (any(sig_coefs)) {
        ...
        if (length(sig_interactions) > 0) {
          ...
          p_values_interactions <- numeric(0)
          sig_interactions_dropdup <- character(0)
        }
      } else {
        sig_coefs <- character(0)
        sig_indices <- integer(0)
        sig_ests <- numeric(0)
        
        sig_interactions <- character(0)
        sig_interactions_indices <- integer(0)
        sig_interactions_ests <- numeric(0)
        p_values_interactions <- numeric(0)
        sig_interactions_dropdup <- character(0)
      }
