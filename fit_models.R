...
check_multicollinearity <- function(data, fixed_effects, snp_columns, 
  ...
  return(list(final_fixed_effects = final_fixed_effects, 
              final_snp_columns = final_snp_columns))
}


# Function to generate model formulas (includes multicollinearity check)
generate_formulas <- function(data, fixed_effects_formula, 
                              snp_columns, gene, log_file) {
  ...
  null_formula <- paste("expres ~ (1 | donor_id) + (1 | pool_num) +", 
