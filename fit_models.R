...
check_multicollinearity <- function(data, fixed_effects, snp_columns, gene, threshold = 10, log_file) {
...
if (is.na(num_cores)) num_cores <- detectCores() - 1  # Fallback if not running on SLURM
...
clusterExport(cl, c("genes_list", "counts_matrix", "nUMI", "perc_mt", "age", 
                    ...
                    "check_multicollinearity", "vif_func"))
...
process_gene <- function(i, assigned_genes, start_time) {
  ...
  result <- tryCatch({
    withCallingHandlers({
      ...
      lrt_null_vs_full <- anova(null_model, full_model)
      lrt_full_vs_interaction <- anova(full_model, interaction_model)
      
      # Extracting LRT statistics
      lr_stat_null_vs_full <- lrt_null_vs_full[2, "Chisq"]
      p_val_null_vs_full <- lrt_null_vs_full[2, "Pr(>Chisq)"]
      lr_stat_full_vs_interaction <- lrt_full_vs_interaction[2, "Chisq"]
      p_val_full_vs_interaction <- lrt_full_vs_interaction[2, "Pr(>Chisq)"]
      ...
      model_comparison <- data.frame(
        ...
        lr_stat_null_vs_full = lr_stat_null_vs_full,
        p_val_null_vs_full = p_val_null_vs_full,
        lr_stat_full_vs_interaction = lr_stat_full_vs_interaction,
        p_val_full_vs_interaction = p_val_full_vs_interaction
      )
...
fitted_df_null <- data.frame()
fitted_df_full <- data.frame()
fitted_df_interaction <- data.frame()
...
for (i in seq_along(results)) {
  ...
  if (!is.null(result$error)) {
    ...
    fitted_df_null <- rbind(fitted_df_null, t(result$fitted_vals_null))
    fitted_df_full <- rbind(fitted_df_full, t(result$fitted_vals_full))
    fitted_df_interaction <- rbind(fitted_df_interaction, t(result$fitted_vals_interaction))
    model_comparison_df <- rbind(model_comparison_df, result$model_comparison)
  }
...
cells <- onek1k@assays[["RNA"]]@counts@Dimnames[[2]]

colnames(fitted_df_null) <- cells
colnames(fitted_df_full) <- cells
colnames(fitted_df_interaction) <- cells
